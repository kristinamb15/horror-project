...
death_urls = pd.read_csv(path.join(raw_data, '03_death_list_urls_all.csv'), index_col=0)
wiki_urls = pd.read_csv(path.join(raw_data, '03_horror_wiki_urls_all.csv'), index_col=0)
...
death_urls.to_csv(path.join(raw_data, '04_death_list_urls_stripped.csv'))
...
wiki_urls.to_csv(path.join(raw_data, '04_horror_wiki_urls_stripped.csv'))

movies = pd.read_csv(path.join(raw_data, '02_horror_movies_clean_year.csv'), index_col=0)
movies['Stripped Title'] = movies.apply(lambda x: strip_title(x['Title']), axis=1)
movies.to_csv(path.join(raw_data,'04_horror_movies_stripped.csv'))
