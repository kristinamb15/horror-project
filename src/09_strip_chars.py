...
df1 = pd.read_csv(path.join(raw_data, '07_horror_characters_clean.csv'), index_col=[0,1])
df1['stripped character'] = df1.apply(lambda x: strip_name(x['character']), axis=1)
df1.to_csv(path.join(raw_data, '09_horror_characters_stripped.csv'))

df2 = pd.read_csv(path.join(raw_data, '08_horror_wiki_deaths_all.csv'), index_col=[0,1])
df2['stripped character'] = df2.apply(lambda x: strip_name(x['character']), axis=1)
df2.to_csv(path.join(raw_data, '09_horror_wiki_deaths_stripped.csv'))

df3 = pd.read_csv(path.join(raw_data, '08_list_wiki_deaths_all.csv'), index_col=[0,1])
df3['stripped character'] = df3.apply(lambda x: strip_name(x['character']), axis=1)
df3.to_csv(path.join(raw_data, '09_list_wiki_deaths_stripped.csv'))
